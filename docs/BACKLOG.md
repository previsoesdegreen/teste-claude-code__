...
| ID | Módulo | Descrição | Dependências | Prioridade | Status | Evidência de conclusão |
|----|--------|-----------|---------------|------------|--------|--------------------------|
| BKL-104 | Qualidade | Revisão de acessibilidade básica (labels, contraste, navegação por teclado) em `index.html`/`css/styles.css` | BKL-001..BKL-011 | Média | CONCLUIDO | 9 pares de contraste corrigidos (calculados pela fórmula WCAG, todos ≥4.5:1 em ambos os temas); labels adicionados a busca/filtro; modal de exclusão com `role="dialog"`/ARIA e gerenciamento de foco (`Esc` fecha, foco retorna ao gatilho); `aria-label` contextual nos botões de linha. Validado no navegador (foco, teclado, recontagem de contraste) + regressão (16/16 testes automatizados, fluxos manuais, sem erros de console) — ver `QA-REPORT.md`, Ciclo 6 |
| BKL-105 | Segurança | Revisão formal de segurança client-side da Fase 2 (reexecução do checklist de `SECURITY-REPORT.md` contra o estado atual do código) | BKL-001..BKL-011 | Média | CONCLUIDO | Achado encontrado e corrigido: `fmtDate()` sem `escapeHtml()` nas colunas de data (auto-XSS local via `localStorage` adulterado). Corrigido em `js/app.js`; validado com PoC (`onerror` deixou de disparar) e regressão completa (16/16 testes automatizados + fluxos manuais) — ver `QA-REPORT.md`, Ciclo 5, e `SECURITY-REPORT.md`, seção 4 |
